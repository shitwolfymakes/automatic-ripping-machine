...
`extra="allow"` everywhere: rows written before migration 0031 (and the
transitional `pending_session_id` mirror the UIs still read) must survive a
validate/dump round-trip. Tighten to `forbid` when the mirror dies and the
wire exposes this model (the step-2 lockdown slice).
"""
