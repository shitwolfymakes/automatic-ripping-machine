...
- **Origin validation at upgrade.** Browsers send `Origin` on WS handshakes but do *not* enforce same-origin the way they do for XHR — a malicious page could otherwise open a WS to Backend while a logged-in user is on another tab. Backend accepts an `Origin` that matches the host the socket was opened against (the request's `Host`, or `X-Forwarded-Host`/`X-Forwarded-Proto` set by the UI's own nginx and the vite dev proxy — a browser can forge none of these), so any hostname, LAN IP or port a deployment is actually reached at works with zero config. `ARM_ALLOWED_ORIGINS` in `.env` allowlists extra origins for split-origin topologies (an external reverse proxy in front). Service-token connections skip this check (they come from sibling containers with no `Origin`).
