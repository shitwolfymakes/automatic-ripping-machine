...
7. **Known cosmetic bug:** despite `tags: type=raw,value=latest,enable=${{ !contains(github.ref_name, '-') }}` rendering as `enable=false` for `v3.0.0-alpha-2`, `docker/metadata-action@v5` published `:latest` anyway (same digest as the alpha tag). Harmless on a fork that only has alphas; needs `flavor: latest=auto` + `type=semver` switch before stable cut so pre-release tags don't move the moving tag.
