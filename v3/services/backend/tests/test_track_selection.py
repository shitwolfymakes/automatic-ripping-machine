...
def test_archive_returns_every_title():
    ...
    tracks = select_tracks("job_x", scan, _preset(TrackSelection.ARCHIVE))
...
def test_all_tracks_filters_below_minimum():
    ...
    tracks = select_tracks("job_x", scan, _preset(TrackSelection.ALL_TRACKS))
...
def test_main_feature_returns_longest_above_threshold():
    ...
    tracks = select_tracks("job_x", scan, _preset(TrackSelection.MAIN_FEATURE))
    assert len(tracks) == 1
...
def test_empty_scan_returns_empty():
    tracks = select_tracks("job_x", _scan(), _preset(TrackSelection.ARCHIVE))
    assert tracks == []
