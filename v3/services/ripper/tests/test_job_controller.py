...
async def test_job_deleted_ws_event_wipes_raw_dir(tmp_path):
    """Backend's DELETE /api/jobs/{id}?delete_raw=true emits `job.deleted`
    on the drive's commands topic; the ripper handler runs rmtree even
    though there's no active task to cancel."""
    raw_dir = tmp_path / "job_gone"
    raw_dir.mkdir()
    (raw_dir / "title00.mkv").write_bytes(b"partial")
    ...
    envelope = WSEnvelope(
        ...
        topic="ripper.commands.drv_test",
        job_id="job_gone",
        payload={"job_id": "job_gone", "drive_id": "drv_test", "delete_raw": True},
    )
    await controller.on_ws_command(envelope)
...
async def test_job_deleted_ws_event_missing_job_id_is_logged(caplog):
    """A malformed payload must not crash the ripper — log + ignore."""
    controller = JobController(FakeClient(), "drv_test")
    envelope = WSEnvelope(
        event_id="evt_test",
        event_type="job.deleted",
        ...
        topic="ripper.commands.drv_test",
        job_id=None,
        payload={"drive_id": "drv_test"},
    )
    with caplog.at_level("WARNING", logger="arm_ripper.job_controller"):
        await controller.on_ws_command(envelope)
    assert any("job.deleted without job_id" in r.message for r in caplog.records)
