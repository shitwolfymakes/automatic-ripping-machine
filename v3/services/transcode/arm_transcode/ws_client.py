"""Long-lived WS connection from transcoder to Backend.

Mirrors the ripper's `ws_client.py` reconnect-with-backoff + replay-
subscriptions pattern, but adds the `X-ARM-Task-Id` header so the
Backend's principal resolver knows this is a transcoder (not a ripper)
service-token connection.

`publish` is fire-and-forget — `transcode.progress.{task_id}` is
telemetry. Losing a tick during a Backend restart is invisible to the UI;
the next REST heartbeat replaces it.
"""
...
logger = logging.getLogger("arm_transcode.ws_client")
...
class WSClient:
    ...
    async def __aenter__(self) -> WSClient:
        self._task = asyncio.create_task(self._connect_loop(), name="ws-transcoder-loop")
        return self
    ...
    async def _run_once(self) -> None:
        async with websockets.connect(
            ...
            additional_headers=[
                ("X-ARM-Hostname", self._hostname),
                ("X-ARM-Task-Id", self._task_id),
            ],
            ping_interval=20.0,
            ping_timeout=20.0,
            max_size=2**20,
            ...
            logger.info("ws transcoder connected url=%s task_id=%s", self._url, self._task_id)
