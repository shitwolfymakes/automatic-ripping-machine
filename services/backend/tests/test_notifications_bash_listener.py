...
def _msg(*, enabled: bool = True, event_type: str = "rip.completed") -> Message:
    return Message(
        ...
        default_body="{job_title} done ({tracks_done}/{tracks_total}).",
        job=_job(),
        payload={"tracks_done": 3, "tracks_total": 3},
        apprise_enabled=enabled,
    )


def _channel(
    script: str, *, enabled: bool = True, events=("rip.completed",), templates=None, inputs=None
) -> NotificationChannel:
    return NotificationChannel(
        ...
        config={
            ...
            "script": script,
            "timeout_seconds": 5,
            "inputs": inputs if inputs is not None else {"TO": "me@x"},
            "secret_keys": [],
        },
...
async def test_runs_script_and_logs_success(tmp_path: Path) -> None:
    ...
    assert (
        log.success
        and log.channel_id == "ncl_bash"
        and log.event_id == "evt_1"
        and log.title == "ARM: rip completed - Dune"
    )
...
async def test_skips_disabled_unsubscribed_and_other_types(tmp_path: Path) -> None:
    ...
    db.rows["notification_channels"] = [
        ...
        NotificationChannel(
            ...
            name="a",
            enabled=True,
            config={"type": "apprise", "url": "json://x"},
