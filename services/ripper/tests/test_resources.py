...
def test_gather_resources_shape(tmp_path):
    roots = {"RAW_ROOT": str(tmp_path)}
    with patch.object(rr, "probe_cpu_percent", return_value=7.0), \
...
def test_gather_resources_skips_missing_root():
    roots = {"NOPE": "/no/such/path/xyz"}
    with patch.object(rr, "probe_cpu_percent", return_value=1.0), \
         patch.object(rr, "probe_cpu_temp", return_value=0.0):
        snap = rr.gather_resources(roots)
    assert snap.storage == []
