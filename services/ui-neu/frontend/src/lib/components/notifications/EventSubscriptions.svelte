<script lang="ts">
	...
	import {
		EVENT_KEYS, EVENT_LABELS, EVENT_VARIABLES, EVENT_DEFAULT_TEMPLATES,
		FIELD_INPUT_CLASS, type EventKey
	...
	let {
		selected = $bindable(),
		templates = $bindable()
	}: { selected: string[]; templates: Record<string, ChannelTemplate> } = $props();
	...
	function varsFor(key: string): string[] { return EVENT_VARIABLES[key as EventKey] ?? []; }
	...
	{#each EVENT_KEYS as key}
		<div class="rounded-md border border-primary/15 bg-page p-3 dark:border-primary/20 dark:bg-primary/5">
			<label class="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
				...
				/>
				<span class="font-medium">{EVENT_LABELS[key]}</span>
			</label>
				...
				<div class="mt-3 space-y-2 pl-6">
					<label class="flex flex-col gap-1">
						...
						<input
							...
							oninput={(e) => { ensure(key).title = (e.currentTarget as HTMLInputElement).value || null; rememberCaret(key, 'title', e.currentTarget as HTMLInputElement); }}
					...
					<label class="flex flex-col gap-1">
						...
						<textarea
							...
							oninput={(e) => { ensure(key).body = (e.currentTarget as HTMLTextAreaElement).value || null; rememberCaret(key, 'body', e.currentTarget as HTMLTextAreaElement); }}
