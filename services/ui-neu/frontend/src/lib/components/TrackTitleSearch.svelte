...
<div class="space-y-3 rounded-md border border-primary/15 bg-primary/5 p-3 dark:border-primary/20 dark:bg-primary/10">
	...
	{#if searchError}
		...
		{:else}
			<div class="flex items-center gap-3">
				...
				{#if query.trim()}
					<button
						onclick={() => { detail = { title: query.trim(), year: yearInput.trim() ? Number(yearInput.trim()) : null, kind: 'movie', poster_url: null, provider_id: null }; searchError = null; }}
	...
	{#if !detail && results.length > 0}
		<div class="flex flex-wrap items-stretch gap-1.5">
			...
			{/each}
			<button
				onclick={() => { detail = { title: query.trim(), year: yearInput.trim() ? Number(yearInput.trim()) : null, kind: 'movie', poster_url: null, provider_id: imdbInput.trim() || null }; }}
