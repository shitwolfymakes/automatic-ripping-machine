...
The full reversion narrative lives at [docs/plans/MASTER_IMPLEMENTATION_PLAN.md § Phase 15.5](../../docs/plans/MASTER_IMPLEMENTATION_PLAN.md). The original `rip_title` and `_wait_for_drive_ready` code can be recovered from git history before commit `<this-commit>` if comparison context is ever needed.
