...
- Anything the ripper does that *seems* to need elevated privilege: first check whether it survives the `gosu arm` drop. The container runs unprivileged by design (the `arm-ripper-srN` service that [devtools/setup-dev.sh](../../devtools/setup-dev.sh) generates via `emit_ripper_block`: no `cap_add`, no `security_opt`, just `group_add: ${CDROM_GID}`) — keep it that way.
